...
static DKCGameCoreResult boot_core(const DKCGameCoreBootConfig *config,
  ...
  *out_instance = NULL;
  ...
  if (config->rom_size != kRomSize || config->rom_size > INT_MAX ||
    ...
    set_error(config, "ROM size does not match generated DKC1 code");
  ...
  if (memcmp(digest, dkc_compiled_rom_sha256, sizeof digest) != 0) {
    set_error(config, "ROM does not match generated DKC1 code");
    return DKC_GAME_CORE_ROM_REJECTED;
  }
  if (s_boot_attempted) {
    set_error(config, s_booted
                         ? "DKC1 core is already booted"
                         : "DKC1 core boot already failed; restart the process");
  ...
  if (!SnesInit(config->rom_bytes, (int)config->rom_size)) {
    set_error(config, "DKC1 SnesInit failed");
...
const DKCGameCoreV2 *dkc1_game_core(void) {
  return &kCore;
}
